...
- **AI Agent 漫画工具** — 注册三个 AstrBot Tool：`suwayomi_search_manga`（自然语言跨源搜索）、`suwayomi_get_chapters`（章节查询，支持 `latest`/`list`/章节号/`ID:数字`）、`suwayomi_send_chapter`（仅明确阅读意图时发送，默认 PDF，支持 ZIP/CBZ/图片，同回合去重）。新增 `suwayomi/ai_service.py`（无副作用服务层）和 `suwayomi/ai_tools.py`（`SuwayomiFunctionTool` 子类，`call()` 覆写保证配置重载后绑定稳定）。PR #9。
- **AI 交互状态管理** — `AiInteractionState` 按 `(unified_msg_origin, sender_id)` 隔离章节候选和发送回执；`asyncio.Lock` 防并发发送；`after_message_sent` 钩子联动 `/reset` 自动清除会话状态。
...
- **回归测试** — `tests/test_ai_service.py`（14 个）、`tests/test_ai_tools.py`（4 个）、Web API 配置边界测试。
