...
- **旧配置迁移覆盖新填值** — 配置曾被 Core 清空后重新填写分组配置的用户，重载插件时 Core 补回的 invisible 旧键默认值会被迁移逻辑误当作"用户旧值"覆盖分组中的真实配置；迁移改为**仅迁移非默认值的旧键**（等于 `_LEGACY_KEY_DEFAULTS` 的旧键视为无意义占位，原样保留），并保证迁移标记在首次评估时即持久化保存
...
- **配置项按功能分组** — `_conf_schema.json` 配置改为六组嵌套结构（`server` 服务器连接 / `reading` 阅读体验 / `pack` 下载打包 / `push` 自动推送 / `ai` AI 漫画工具 / `advanced` 高级），与 WebUI 设置页分组一致；新增 `suwayomi/config.py` 提供分组读写与兼容迁移
- **旧版平铺配置无损迁移** — `_conf_schema.json` 中保留全部旧配置键（`invisible: true`，对用户不可见），使 AstrBot Core 的配置同步不会删除旧键、用户配置值得以保留；插件加载时 `migrate_legacy_config()` 把旧值迁入分组并置迁移标记 `_config_migrated_v1`，标记置位后不再迁移（防止 Core 补回的默认值覆盖真实配置）。迁移完成的下一个大版本可移除 schema 中的兼容旧键
...
- **「漫画 搜索」默认源选择与 AI 工具统一** — 默认搜索不再取原始 `sources[:5]`（本地源 + MangaDex 多语言变体挤占）；`search_manga` 与批量订阅的 `search_best_match` 改用 `select_search_sources`：排除本地源（ID "0"）、优先不同扩展名、语言变体补位，与 AI Agent 路径行为一致
- **「漫画 搜索」源名匹配语义** — 源提示改为前缀/语言代码匹配（`match_source_hint`，排除本地源），不再子串匹配（避免"漫画"等常见词误锁单源）；末词不是源名时自动拼回关键词并提示；多词标题用 `+` 连接（如 `香格里拉+再`），`client.search_manga` 将 `+` 转回空格传给源；`select_search_sources`/`_bounded_int` 从 `ai_service` 移至 `service`（消除反向依赖）
