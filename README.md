<p align="center">
  ...
  <p align="center">
    ...
    <img src="https://img.shields.io/badge/version-0.6.1-8A2BE2?style=flat-square" alt="Version 0.6.1">
...
> 开启 `result_cards_enabled` 后，搜索 / 订阅 / 章节列表 / 更新通知等指令结果会通过 AstrBot T2I 服务渲染为带封面的卡片；章节列表、订阅确认、更新通知卡片会顺带展示漫画简介。

|  |  |
|---|---|
| ![搜索](docs/assets/漫画%20搜索%20致彼时繁花.jpg) | ![章节列表](docs/assets/漫画%20章节%20月不会数羊.jpg) |
| **🔍 搜索** | **📖 章节列表** |
| ![订阅确认](docs/assets/漫画%20订阅%204.jpg) | ![更新通知](docs/assets/漫画%20更新通知.jpg) |
| **✅ 订阅确认** | **🔔 更新通知** |
| ![批量订阅](docs/assets/漫画%20批量订阅.jpg) | ![我的订阅](docs/assets/漫画%20我的订阅.jpg) |
| **📚 批量订阅** | **📋 我的订阅** |
...
| `/漫画 推送 开/关/状态` | 控制自动推送（开记住偏好，新订阅自动继承；关清除偏好） |
| `/漫画 帮助` | 显示完整用法说明 |
...
| `suwayomi_subscribe_manga` | 订阅漫画更新通知，可选开启自动推送（`push_enabled`，不传时继承会话偏好）。已订阅时可升级或降级推送 |
...
默认只发送文本通知。开启自动推送后，发现更新时会自动发送漫画内容。开启后会记住偏好，之后通过指令新订阅的漫画自动开启推送：
...
| `t2i_source` | string | `system` | T2I 服务来源：`system`（使用 AstrBot 系统 T2I 配置）/ `custom`（单独配置，仅本插件使用下面的端点） |
...
> 💡 **卡片渲染**：插件默认以纯文本回复。如需将指令结果渲染为带封面的精美卡片（章节列表、订阅确认、更新通知卡片会同时展示漫画简介），在 WebUI 插件设置中将 `result_cards_enabled` 开启即可。默认使用 AstrBot 系统 T2I 配置（通常随 AstrBot 内置可用）；如需与系统配置隔离，可将 `t2i_source` 设为 `custom` 并填写 `t2i_endpoint`，自建服务部署方式见 [docs/setup.md](docs/setup.md#可选自建-t2i-卡片渲染服务)。T2I 服务不可用时所有命令自动回退纯文本，不会报错。
